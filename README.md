...
skill-switch 内置以下工具的发现目录（可在「设置」中启用 / 禁用或自定义路径）：
...
- **主动联网边界**：应用只在你主动「从 GitHub 安装 Skill」时通过 `git` 拉取公开仓库内容。其他操作（扫描、整理、部署、取消部署、批量操作、备份与恢复）完全在本机完成，不会把你的 Skill 内容或本机路径上传到任何服务器。
...
- **安全的批量操作**：批量部署、按工具批量取消部署和批量从注册表移除逐项给出结果，失败项可重试，不会「全有或全无」。
...
1. **启用工具并扫描**：在「设置」中确认要管理的工具目录，回到「技能」页点「扫描」。
